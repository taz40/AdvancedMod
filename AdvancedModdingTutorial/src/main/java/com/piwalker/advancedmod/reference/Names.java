package com.piwalker.advancedmod.reference;

/**
 * Created by SamuelPiWalker on 7/21/2015.
 */
public class Names {
    public static class Blocks{
        public static String SMILEY = "smileface";
        public static String YELLOW = "blockyellow";
        public static String CAMOMINE = "camoMine";
    }

    public static class Keys{
        public static String net = "keys.advancedmod.net";
        public static String category = "keys.advancedmod.category";
        public static String bigExplode="keys.advancedmod.explodeBig";
    }

    public static class TileEntities{
        public static String CAMOMINE = "camoMineTileEntity";
    }
}
